...
import net.mcxk.hjyhunt.game.GameProgress;
...
public class ProgressDetectingListener implements Listener {
    ...
    public void craftCompass(CraftItemEvent event) {
        ...
        plugin.getGame().getProgressManager().unlockProgress(net.mcxk.hjyhunt.game.GameProgress.COMPASS_UNLOCKED);
    }
    ...
    public void getIron(FurnaceExtractEvent event) {
        ...
        plugin.getGame().getProgressManager().unlockProgress(net.mcxk.hjyhunt.game.GameProgress.IRON_MINED);
    }
    ...
    public void breakStone(BlockBreakEvent event) {
        ...
        plugin.getGame().getProgressManager().unlockProgress(net.mcxk.hjyhunt.game.GameProgress.STONE_AGE);
    ...
    public void changeDim(PlayerPortalEvent event) {
        if (Objects.requireNonNull(Objects.requireNonNull(event.getTo()).getWorld()).getEnvironment() == World.Environment.NETHER) {
            plugin.getGame().getProgressManager().unlockProgress(net.mcxk.hjyhunt.game.GameProgress.ENTER_NETHER, event.getPlayer());
            return;
        }
        if (event.getTo().getWorld().getEnvironment() == World.Environment.THE_END) {
            plugin.getGame().getProgressManager().unlockProgress(net.mcxk.hjyhunt.game.GameProgress.ENTER_END);
        }
    }
    ...
    public void pickup(EntityPickupItemEvent event) {
        if (event.getItem().getItemStack().getType() == Material.ENDER_PEARL) {
            plugin.getGame().getProgressManager().unlockProgress(net.mcxk.hjyhunt.game.GameProgress.GET_ENDER_PEARL);
            return;
        }
