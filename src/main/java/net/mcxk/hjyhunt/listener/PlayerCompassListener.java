...
public class PlayerCompassListener implements Listener {
    ...
    public void craftCompass(CraftItemEvent event) {
        ...
        Optional<net.mcxk.hjyhunt.game.PlayerRole> role = plugin.getGame().getPlayerRole(player);
        if (!role.isPresent()) {
            return;
        }
        ...
        } else if (role.get() == net.mcxk.hjyhunt.game.PlayerRole.RUNNER) {
            // 逃亡者合成，锁回去
    ...
    public void respawnGivenCompass(PlayerRespawnEvent event) {
        if (plugin.getGame().getStatus() == GameStatus.GAME_STARTED && plugin.getGame().isCompassUnlocked()) {
            Optional<net.mcxk.hjyhunt.game.PlayerRole> role = plugin.getGame().getPlayerRole(event.getPlayer());
    ...
    public void inventoryCompass(InventoryEvent event, HumanEntity humanEntity) {
        ...
        Optional<net.mcxk.hjyhunt.game.PlayerRole> role = plugin.getGame().getPlayerRole(player);
        if (!role.isPresent() ||
                role.get() != net.mcxk.hjyhunt.game.PlayerRole.HUNTER ||
