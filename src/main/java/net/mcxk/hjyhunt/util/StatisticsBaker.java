...
public class StatisticsBaker {
    private final Game game = HJYHunt.getInstance().getGame();
    ...
    public String getTeamBadGuy() {
        ...
        for (Map.Entry<Player, Double> playerDoubleEntry : HJYHunt.getInstance().getGame().getTeamDamageData().entrySet()) {
    ...
    public String getTeamEndDragonBadGuy() {
        Player maxPlayer = null;
        double maxDamage = 0.0d;
        for (Map.Entry<Player, Double> playerDoubleEntry : HJYHunt.getInstance().getGame().getTeamDamageEndDragonData().entrySet()) {
            if (playerDoubleEntry.getValue() < maxDamage) {
                continue;
            }
            maxPlayer = playerDoubleEntry.getKey();
